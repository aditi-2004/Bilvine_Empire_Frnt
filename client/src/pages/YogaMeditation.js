...
const YogaMeditation = () => {
  ...
  const fetchYogaMeditationPlan = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://gym-backend-l7s5.onrender.com/api/yoga-meditation?${queryParams}`);
      ...
      // Set plan data from the API response
      setPlanData(data.plan || []);
    } catch (error) {
      console.error('Error fetching yoga/meditation plan:', error);
      setError('Failed to load yoga and meditation plans. Please try again later.');
      setPlanData([]);
    } finally {
      setLoading(false);
    }
      ...
      <div className="plan-section">
        {loading ? (
          ...
          <motion.p
            ...
            className="error-text"
          >
            No yoga or meditation plan available. Check filters or try different selections.
          </motion.p>
        )}
      </div>
          ...
          >
            ...
            >
              <motion.h2
                ...
                transition={{ delay: 0.2, duration: 0.5 }}
              >
                {selectedExercise.name} {selectedExercise.pose && `(${selectedExercise.pose})`}
              </motion.h2>
              <motion.img
                src={selectedExercise.image || '/assets/placeholder.jpg'}
                alt={selectedExercise.name}
